...
void dealResponseAtCmd(struct KernelResponse rsp) {
    ...
    if (rsp.code < 0 || rsp.data == NULL || rsp.header == NULL ||
        rsp.body == NULL)
        return;
    // 处理数据
...
int showOneRule(struct IPRule rule) {
    char saddr[25], daddr[25], sport[13], dport[13], proto[6], action[8],
        log[5];
    // ip
    ...
    printf("| %-*s | %-18s | %-18s | %-11s | %-11s | %-8s | %-6s | %-3s |\n",
           MAXRuleNameLen, rule.name, saddr, daddr, sport, dport, proto, action,
           log);
    printLine(111);
}
...
int showRules(struct IPRule* rules, int len) {
    ...
    printLine(111);
    printf("| %-*s | %-18s | %-18s | %-11s | %-11s | %-8s | %-6s | %-3s |\n",
           MAXRuleNameLen, "name", "source ip", "target ip", "source port",
           "target port", "protocol", "action", "log");
    printLine(111);
    for (i = 0; i < len; i++) {
        showOneRule(rules[i]);
    }
