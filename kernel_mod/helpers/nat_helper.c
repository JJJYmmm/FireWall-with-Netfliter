...
int delNATRuleFromChain(int num) {
    ...
    for (now = natRuleHead, count = 1; now != NULL && now->nx != NULL;
         now = now->nx, count++) {
        if (count == num) {  // 删除规则
            tmp = now->nx;
            now->nx = now->nx->nx;
...
void* formAllNATRules(unsigned int* len) {
    ...
    for (now = natRuleHead, count = 0; now != NULL; now = now->nx, count++)
        ;
    *len =
        sizeof(struct KernelResponseHeader) + sizeof(struct NATRecord) * count;
    ...
    head->arrayLen = count;
    for (now = natRuleHead, p = (mem + sizeof(struct KernelResponseHeader));
         now != NULL; now = now->nx, p = p + sizeof(struct NATRecord))
    ...
    *isMatch = 0;
    ...
    for (now = natRuleHead; now != NULL; now = now->nx) {
        if (isIPMatch(sip, now->saddr, now->smask) &&
            !isIPMatch(dip, now->saddr, now->smask) && dip != now->daddr) {
            read_unlock(&natRuleLock);
            *isMatch = 1;
            return now;
        }
...
// modify!!!
struct NATRecord* matchNATRule_hj(unsigned int sip,
                                  unsigned int dip,
    ...
    *isMatch = 0;
    read_lock(&natRuleLock);
    for (now = natRuleHead; now != NULL; now = now->nx) {
        if (dip == now->daddr && dport == now->dport) {
            read_unlock(&natRuleLock);
            *isMatch = 1;
            return now;
        }
...
struct NATRecord genNATRecord(unsigned int preIP,
                              unsigned int afterIP,
                              unsigned short prePort,
                              unsigned short afterPort) {
