...
int addLogBySKB(unsigned int action, struct sk_buff* skb) {
    ...
    log.sport = sport;
    log.dport = dport;
    // printk("header->tot_len:%d,%d;head->ihl:%d,%d", header->tot_len,
    //        ntohs(header->tot_len), header->ihl, ntohs(header->ihl));
...
void* formAllIPLogs(unsigned int num, unsigned int* len) {
    ...
    if (num == 0 || num > count)
        num = count;
    *len = sizeof(struct KernelResponseHeader) +
           sizeof(struct IPLog) * num;  // 申请回包空间
